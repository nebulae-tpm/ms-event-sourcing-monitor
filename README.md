...
![Intro](docs/images/ms-event-sourcing-monitor_intro.png "Intro")
# Table of Contents
  ...
  * [BackEnd](#backend)
    *  [event-sourcing-monitor](#backend_event-sourcing-monitor)
        * [Environment variables](#backend_event-sourcing-monitor_env_vars)
        * [Event Sourcing](#backend_event-sourcing-monitor_eventsourcing)
        * [CronJobs](#backend_event-sourcing-monitor_cronjobs)
...
│   ├── event-sourcing-monitor                     => Micro-BackEnd responsible for ...
...
Note: You may find the GraphQL schema [here](api/gateway/graphql/event-sourcing-monitor/schema.gql)
...
### GraphQL Queries
#### getHelloWorldFromevent-sourcing-monitor
...
#### event-sourcing-monitorHelloWorldSubscription
...
## event-sourcing-monitor <a name="backend_event-sourcing-monitor"></a>
...


### Environment variables <a name="backend_event-sourcing-monitor_env_vars"></a>
...
```
#### Notes: 
  * ENV VARS for development are [here](backend/event-sourcing-monitor/.env)
  * ENV VARS for production are [here](deployment/gke/deployment-event-sourcing-monitor.yaml)

### Event Sourcing <a name="backend_event-sourcing-monitor_eventsourcing"></a>
...
### CronJobs <a name="backend_event-sourcing-monitor_cronjobs"></a>
...
### 1. clone this repo  
   ```git clone https://github.com/nebulae-tpm/ms-event-sourcing-monitor.git```  
...
### 5. Compose FrontEnd
```
nebulae compose-ui development --shell-type=FUSE2_ANGULAR --shell-repo=https://github.com/nebulae-tpm/emi --frontend-id=emi --output-dir=/FULL_PATH_TO_REPO/ms-event-sourcing-monitor/playground/emi  --setup-file=/FULL_PATH_TO_REPO/ms-event-sourcing-monitor/etc/mfe-setup.json
```

### 6. Compose the API gateway
```
nebulae compose-api development --api-type=NEBULAE_GATEWAY --api-repo=https://github.com/nebulae-tpm/gateway --api-id=gateway --output-dir=FULL_PATH_TO_REPO/ms-event-sourcing-monitor/playground/gateway  --setup-file=FULL_PATH_TO_REPO/ms-event-sourcing-monitor/etc/mapi-setup.json
```
...
* set this key value to the **JWT_PUBLIC_KEY** atribute in the following files: *WORKING_FOLDER*/ms-event-sourcing-monitor/backend/event-sourcing-monitor/.env   *WORKING_FOLDER*/ms-event-sourcing-monitor/playground/gateway/.env  
...
```
cd backend/event-sourcing-monitor/
npm install
npm start
```
